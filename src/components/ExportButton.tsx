...
export default function ExportButton({
  ...
  const generateCSV = (): string => {
    ...
    if (topOverTargetCategories.length > 0) {
      ...
      csv +=
        'Category Name,Category Group,Assigned,Target,Variance,Variance %,Target Type\n';
  ...
  const handleExport = async (format: 'csv' | 'json') => {
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        downloadFile(
          csv,
          `ynab-analysis-${budgetSlug}-${month}-${timestamp}.csv`,
          'text/csv'
        );
      } else {
        ...
        downloadFile(
          json,
          `ynab-analysis-${budgetSlug}-${month}-${timestamp}.json`,
          'application/json'
        );
      }
    } catch (error) {
