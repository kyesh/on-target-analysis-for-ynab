...
export class FirestoreCostTracker {
  ...
  private static checkFreeTierLimits(): void {
    ...
    Object.entries(this.operationCounts).forEach(([operation, count]) => {
      if (operation in limits) {
        ...
        if (percentage > 80) {
          console.warn(
            `Approaching daily ${operation} limit: ${count}/${limit} (${percentage.toFixed(1)}%)`
          );
        }
      }
    });
  ...
  private static calculateProjectedCost(): number {
    ...
    const costs = {
      ...
      deletes:
        (Math.max(0, monthlyProjection.deletes - 600000) * 0.02) / 100000,
    };
...
export class BatchOptimizer {
  ...
  private static calculateOperationCost(
    operations: number,
    type: 'read' | 'write' | 'delete'
  ): number {
    const pricing = {
  ...
  static setupCostAlerts(): void {
    setInterval(
      async () => {
        ...
        Object.entries(report.freeTierUsage).forEach(
          ([operation, percentage]) => {
            if (percentage > 80) {
              this.sendAlert(
                `High ${operation} usage: ${percentage.toFixed(1)}% of daily free tier`
              );
            }
          }
        );
        ...
        if (report.costs.projected_monthly > 50) {
          this.sendAlert(
            `Projected monthly cost: $${report.costs.projected_monthly.toFixed(2)}`
          );
        }
      },
      60 * 60 * 1000
  ...
  static generateOptimizationRecommendations(): string[] {
    ...
    if (cacheStats.hitRate < 0.7) {
      recommendations.push(
        'Increase cache TTL to improve hit rate and reduce read operations'
      );
    }
    ...
    if (stats.daily.deletes > 10000) {
      recommendations.push(
        'Consider running cleanup operations during off-peak hours'
      );
    }
    ...
    if (stats.daily.reads > 30000) {
      recommendations.push(
        'Implement more aggressive caching for frequently accessed sessions'
      );
    }
